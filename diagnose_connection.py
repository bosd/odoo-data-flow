...
def run_diagnostic(config_file: str, num_records: int) -> None:
    ...
    try:
        ...
        if messages:
            console.print(
                "[bold yellow]Warning: Odoo returned messages during load."
                "[/bold yellow]"
            )
            for msg in messages:
            ...
            console.print(
                Panel(
                    "Your connection and server appear to be configured "
                    "correctly for bulk loading.",
                    title="[bold green]Diagnosis: OK[/bold green]",
                )
            )

    except Exception as e:
        console.print("[bold red]Error: The `load` method failed.[/bold red]")
        error_str = str(e)

        if "Expecting value" in error_str:
            console.print(
                Panel(
                    "The server returned a non-JSON response. This is the most "
                    "common cause of import failures with remote servers.\n\n"
                    "[bold]Likely Causes:[/bold]\n"
                    "1. [bold]Proxy Timeout:[/bold] The request took too long and a "
                    "proxy server (like Nginx) cut it off.\n"
                    "2. [bold]Request Size Limit:[/bold] The request was too large "
                    "and was blocked by a proxy (e.g., `client_max_body_size` in "
                    "Nginx).\n"
                    "3. [bold]Odoo Server Error:[/bold] Odoo itself crashed and "
                    "returned an HTML error page.\n\n"
                    "[bold]Next Steps:[/bold]\n"
                    "- Ask your server administrator to check the Odoo and proxy "
                    "logs for errors at the time of this test.\n"
                    "- Try running this script with a smaller number of records "
                    "(e.g., `--records 100`) to see if it succeeds.",
                    title="[bold red]Diagnosis: Probable Proxy/Network Issue"
                    "[/bold red]",
                )
            )
        else:
