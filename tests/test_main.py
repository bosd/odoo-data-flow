...
def test_migrate_command_calls_runner(
    mock_run_export: MagicMock, mock_run_import: MagicMock, runner: CliRunner
) -> None:
    """Tests that the migrate command calls the correct runner function."""
    mock_run_export.return_value = (["id", "name"], [[1, "Test"]])
    with runner.isolated_filesystem():
        with open("src.conf", "w") as f:
            f.write("[Connection]\nhostname=a\ndatabase=b\nlogin=c\npassword=d")
        with open("dest.conf", "w") as f:
            f.write("[Connection]\nhostname=a\ndatabase=b\nlogin=c\npassword=d")
        result = runner.invoke(
            __main__.cli,
            [
