...
def test_pass_2_failure_writes_to_fail_file(tmp_path: Path) -> None:
    """Test Pass 2 failure creates a fail file.
    ...
    assert len(fail_data) == 2, "Fail file should contain a header and one failed row."
...
def test_pass_2_failure_writes_to_fail_file_with_content_check(
    tmp_path: Path,
) -> None:
    """Test that a pass 2 failure creates a faile file with correct content.
    ...
    def write_side_effect(vals: dict[str, Any]) -> Any:
        current_record_db_id = mock_model.browse.call_args.args[0]
        if current_record_db_id == 103:  # db_id for "child_fail"
            raise Exception("Write Permission Error")
        return True
    ...
    with patch(
        ...
        # We now expect a tuple (bool, int) from import_data
        result, _ = import_threaded.import_data(
            config_file="dummy.conf",
            model=model_name,
            unique_id_field="id",
    ...
    # NEW: Assert that the fail file was created and has the correct content.
    assert fail_file.exists(), "The fail file should have been created."

    with open(fail_file) as f:
        reader = csv.reader(f, delimiter=";")
        fail_content = list(reader)

    # Expected header: original header + "_ERROR_REASON"
    expected_header = [*header, "_ERROR_REASON"]
    assert fail_content[0] == expected_header, "Fail file has incorrect header."

    # Expected row: original "child_fail" row + error message
    expected_row = source_data[2] + ["Write Permission Error"]
    assert fail_content[1] == expected_row, "Fail file has incorrect row data."
    assert len(fail_content) == 2, (
        "Fail file should only contain one header and one failed row."
    )
